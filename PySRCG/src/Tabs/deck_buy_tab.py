...
class DeckBuyTab(NotebookTab):
    @property
    def library_selected(self):  # returns a deck OR a program
        return treeview_get(self.deck_library, self.tree_item_dict)

    @property
    def inv_selected_item(self):
        """ID of the index of the selected item"""
        selection = self.deck_inventory_list.curselection()
        if len(selection) is 0:
            return None
        return selection[-1]

    def __init__(self, parent, deck_change_listener):
        super().__init__(parent)
        self.deck_change_listener = deck_change_listener
        ...
        self.deck_library = ttk.Treeview(self, height=20, show="tree")
        self.deck_library_scroll = ttk.Scrollbar(self, orient=VERTICAL, command=self.deck_library.yview)
        ...
        # inventory list
        self.deck_inventory_list = Listbox(self, width=40)
        self.inventory_list_scroll = ttk.Scrollbar(self, orient=VERTICAL, command=self.deck_inventory_list.yview)
        ...
        self.deck_library.bind("<<TreeviewSelect>>", self.on_tree_item_click)
        self.deck_library["yscrollcommand"] = self.deck_library_scroll.set
        self.deck_inventory_list.bind("<<ListboxSelect>>", self.on_inv_item_click)
        self.desc_box["yscrollcommand"] = self.desc_box_scroll.set
        self.deck_inventory_list["yscrollcommand"] = self.inventory_list_scroll.set

        # fill the treeview with items
        recursive_treeview_fill(self.parent.game_data["Decks"], "", self.deck_library,
                                decking_tab_recurse_check, decking_tab_recurse_end_callback)

        # grids
        self.deck_library.grid(column=0, row=0, sticky=(N, S))
        self.deck_library_scroll.grid(column=1, row=0, sticky=(N, S))
        self.desc_box.grid(column=2, row=0, sticky=(N, S))
        self.desc_box_scroll.grid(column=3, row=0, sticky=(N, S))
        self.deck_inventory_list.grid(column=4, row=0, sticky=(N, S))
    ...
    def on_tree_item_click(self, event):
        ...
        selected = self.deck_library.selection()[-1]
    ...
    def add_deck_item(self, item: Deck):
        self.statblock.decks.append(item)
        self.deck_inventory_list.insert(END, item.name)
        self.deck_change_listener()

    def remove_deck_item(self, index):
        del self.statblock.decks[index]
        self.deck_inventory_list.delete(index)
        self.deck_change_listener()
    ...
    def on_inv_item_click(self, event):
        item_report = self.statblock.decks[self.deck_inventory_list.curselection()[-1]].report()
        self.fill_description_box(item_report)

    def on_buy_click(self):
        # make copy of the item from the dict
        item = treeview_get(self.deck_library, self.tree_item_dict)
        if item is not None:
    ...
    def on_sell_click(self):
        ...
        if len(self.deck_inventory_list.curselection()) is 0:
            return
        selected_item = self.statblock.decks[self.inv_selected_item]

        # return cash value
        self.statblock.cash += selected_item.cost

        self.remove_deck_item(self.inv_selected_item)

    def on_switch(self):
        pass

    def load_character(self):
        self.deck_inventory_list.delete(0, END)
        for item in self.statblock.decks:
            self.deck_inventory_list.insert(END, item.name)